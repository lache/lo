...
void litehtml::document::create_node(GumboNode* node, elements_vector& elements) {
    ...
    case GUMBO_NODE_ELEMENT:
    {
        ...
        for (unsigned int i = 0; i < node->v.element.attributes.length; i++) {
            ...
            const char* gumbo_attr_value = attr->value;
            if (m_context->is_in_loop()) {
                gumbo_attr_value = m_context->get_loop_var(gumbo_attr_value);
            }

            attrs[tstring(litehtml_from_utf8(attr->name))] = litehtml_from_utf8(gumbo_attr_value);
        }
        ...
        if (ret) {
            ...
            int repeat = 1;
            auto loop_attr = attrs.find("lw:loop");
            if (loop_attr != attrs.end()) {
                repeat = m_context->begin_loop(loop_attr->second.c_str());
            }
            ...
            for (int r = 0; r < repeat; r++) {
                ...
                if (loop_attr != attrs.end()) {
                    m_context->next_loop_element(loop_attr->second.c_str());
                }
            }
            elements.push_back(ret);

            if (loop_attr != attrs.end()) {
                m_context->end_loop(loop_attr->second.c_str());
            }
    ...
    {
        const char* gumbo_text = node->v.text.text;
        if (m_context->is_in_loop()) {
            gumbo_text = m_context->get_loop_var(gumbo_text);
        }

        std::wstring str;
        std::wstring str_in = (const wchar_t*)(utf8_to_wchar(gumbo_text));
