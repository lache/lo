...
static void render_item_node(const LWCONTEXT *pLwc,
                             int index,
                             const char *rank,
                             const char *nickname,
                             const char *score,
    ...
    const LWNODERENDER render_c0 = convert_node_to_render(pLwc, YGNodeGetChild(node, 0));
    const LWNODERENDER render_c1 = convert_node_to_render(pLwc, YGNodeGetChild(node, 1));
    const LWNODERENDER render_c2 = convert_node_to_render(pLwc, YGNodeGetChild(node, 2));
    ...
    SET_COLOR_RGBA_FLOAT(text_block.color_emp_glyph, 1, 1, 0, ui_alpha);
    SET_COLOR_RGBA_FLOAT(text_block.color_emp_outline, 0, 0, 0, ui_alpha);
    text_block.text_block_y = render.y - render.h - render.h * index;

    text_block.text = rank;
    text_block.text_bytelen = (int) strlen(text_block.text);
    text_block.begin_index = 0;
    text_block.end_index = text_block.text_bytelen;
    text_block.text_block_x = render_c0.x;
    render_text_block(pLwc, &text_block);
    ...
    text_block.begin_index = 0;
    text_block.end_index = text_block.text_bytelen;
    text_block.text_block_x = render_c1.x;
    render_text_block(pLwc, &text_block);
    ...
    text_block.begin_index = 0;
    text_block.end_index = text_block.text_bytelen;
    text_block.text_block_x = render_c2.x;
    render_text_block(pLwc, &text_block);
}
...
static void render_leaderboard_page(const LWCONTEXT *pLwc, float x_center, float y_bottom, float ui_alpha) {
    ...
    text_block.text_block_y = y_bottom;
    render_text_block(pLwc, &text_block);
}
...
static void render_leaderboard_page_button(const LWCONTEXT *pLwc, float x_center, float y_bottom, float ui_alpha) {
    ...
    const float page_button_w =
            pLwc->viewport_aspect_ratio > 1 ? (pLwc->viewport_rt_x * 0.7f) : (pLwc->viewport_rt_x * 1.0f);
...
void render_leaderboard_table_node(const LWCONTEXT *pLwc, YGNodeConstRef node, float ui_alpha) {
    const LWNODERENDER render = convert_node_to_render(pLwc, node);
    const float x0 = render.x;
    const float y0 = render.y;
    YGNodeConstRef node_first_row = YGNodeGetChild(node, 0);
    const LWNODERENDER render_first_row = convert_node_to_render(pLwc, node_first_row);
    // Render leaderboard table header
    render_item_node(pLwc, 0, "#", "Nickname", "Score", 1, node_first_row, ui_alpha, 1, 1, 0);
    ...
    for (int i = 0; i < p->Count; i++) {
        ...
        render_item_node(pLwc, i + 1, rank_str, p->Nickname[i], score_str, 0, node_first_row, ui_alpha, r, g, b);
        if (i < p->Count - 1) {
    ...
    if (pLwc->viewport_aspect_ratio > 1) {
        ...
        render_leaderboard_page(pLwc,
                                ...
                                render_first_row.y - render_first_row.h * p->Count,
                                ui_alpha);
        render_leaderboard_page_button(pLwc,
                                       -pLwc->viewport_rt_x / 2,
                                       render_first_row.y - render_first_row.h * (p->Count + 1),
                                       ui_alpha);
    } else {
        // render page texts (i.e. '23 / 1023')
        render_leaderboard_page(pLwc,
                                0,
                                render_first_row.y - render_first_row.h * (p->Count + 2),
                                ui_alpha);
        render_leaderboard_page_button(pLwc,
                                       0,
                                       render_first_row.y - render_first_row.h * (p->Count + 4),
                                       ui_alpha);
    }
}
