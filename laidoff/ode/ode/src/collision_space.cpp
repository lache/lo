...
void dxHashSpace::collide (void *data, dNearCallback *callback)
{
    ...
    int tested_rowsize = (int)((n+7) >> 3);	// number of bytes needed for n bits
                            ...
                            if (node->aabb->level == level &&
                                node->x == xi && node->y == yi && node->z == zi) {
                                    ...
                                    if (aabb->index <= node->aabb->index) {
                                        i = (int)((aabb->index * tested_rowsize)+(node->aabb->index >> 3));
                                        mask = 1 << (node->aabb->index & 7);
                                    }
                                    else {
                                        i = (int)((node->aabb->index * tested_rowsize)+(aabb->index >> 3));
                                        mask = 1 << (aabb->index & 7);
                                    }
