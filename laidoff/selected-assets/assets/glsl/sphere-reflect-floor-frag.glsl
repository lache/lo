...
uniform sampler2D diffuse_arrow;
...
void main()
{
	...
	vec3 power = vec3(15.0, 15.0, 15.0);
	vec3 offset = vec3(0.1, 0.1, 0.1);
	vec3 rs = vec3(1.0,1.0,1.0);
	vec3 w = vec3(0.5, 0.5, 0.5) / reflect_size;
	vec3 r = 1.0 / (1.0 + exp(power * (w*d - offset * reflect_size)));
	
	fragColor.rgb += r[0] * sphere_col[0] * sphere_col_ratio[0];
	fragColor.rgb += r[1] * sphere_col[1] * sphere_col_ratio[1];
	fragColor.rgb += r[2] * sphere_col[2] * sphere_col_ratio[2];
}
