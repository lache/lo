...
static inline void mat4x4_rotate(mat4x4 R, const mat4x4 M, float x, float y, float z, float angle)
{
	float s = sinf(angle);
	float c = cosf(angle);
	vec3 u = {x, y, z};
...
static inline void mat4x4_from_quat_skin(mat4x4 M, const quat q) {
    float a = q[3];
    float b = q[0];
    float c = q[1];
    float d = q[2];
...
static inline void quat_from_mat4x4_skin(quat q, const mat4x4 M) {
    float r = 0.f;
    int i;
